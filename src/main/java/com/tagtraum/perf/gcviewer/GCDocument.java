...
public class GCDocument extends JInternalFrame {
	...
	private List<SimpleChart> charts = new ArrayList<>();
	...
	public void addChartToZoom(SimpleChart chart){
		charts.add(chart);
	}
	...
	public void relayout() {
		...
		scaleModelChart();
		scaleAdditionalChart();
		invalidate();
	}

	private void scaleAdditionalChart() {
		for (SimpleChart chart : charts) {
			chart.setRunningTime(getMaxRunningTime());
		}
	}
	...
	private class MultiModelChartFacade implements ModelChart {
		...
		public void autoSetScaleFactor() {
			for (ChartPanelView chartPanelView : chartPanelViews) {
				chartPanelView.getModelChart().autoSetScaleFactor();
			}
		}
