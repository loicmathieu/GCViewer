...
public class GCViewerGui extends JFrame {
	...
	public void add(final File[] files) {
		try {
			if (files.length >= 0) {
				openFileAction.setSelectedFile(files[0]);
			}
			final URL[] urls = convertFilesToURLs(files);
			add(urls);
		} catch (RuntimeException e) {
	...
	private JToolBar initToolBar() {
		...
		zoomComboBox = new JComboBox<String>(new String[] {"Scale to fit", "1%", "5%", "10%", "50%", "100%", "200%", "300%", "500%", "1000%", "5000%"});
