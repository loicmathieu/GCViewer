...
public class ModelPanel extends JTabbedPane {
	...
	private class PauseTab extends ValuesTab {
		...
		public PauseTab() {
			...
			addEntry("Full GC Freed Mem/min");//TODO localize
			...
			addEntry("GC Freed Mem/min");//TODO localize
		}

		public void setModel(GCModel model) {
			//LMA add freed mem/min
			...
			updateValue(LocalisationHelper.getString("data_panel_min_max_full_gc_pause"),
					fullGCDataAvailable ? pauseFormatter.format(model.getFullGCPause().getMin()) + "s / " + pauseFormatter.format(model.getFullGCPause().getMax()) + "s" : "n/a",
							fullGCDataAvailable);
			double fullGcFreedMemPerMin = model.getFreedMemoryByFullGC().getSum() / (model.getRunningTime() / 60);
			updateValue("Full GC Freed Mem/min", freedMemoryPerMinFormatter.format(fullGcFreedMemPerMin) + "/min", true);//TODO localize
			...
			updateValue(LocalisationHelper.getString("data_panel_min_max_gc_pause"),
					gcDataAvailable ? pauseFormatter.format(model.getGCPause().getMin()) + "s / " + pauseFormatter.format(model.getGCPause().getMax()) + "s": "n/a",
							gcDataAvailable);
			double gcFreedMemPerMin = model.getFreedMemoryByGC().getSum() / (model.getRunningTime() / 60);
			updateValue("GC Freed Mem/min", freedMemoryPerMinFormatter.format(gcFreedMemPerMin) + "/min", true);//TODO localize
		}
	}
	...
	private class SummaryTab extends ValuesTab {
		...
		public SummaryTab() {
			super();
			//LMA : add percentage of GC + Young to Full GC percentage
			newGroup("Global", true);//TODO localize
			...
			newGroup("All GC", true);//TODO localize
			addEntry(LocalisationHelper.getString("data_panel_count_gc_pauses"));
			addEntry(LocalisationHelper.getString("data_panel_performance_gc"));
			addEntry("Percentage of time in GC");//TODO localize

			newGroup("Full GC", true);//TODO localize
			addEntry(LocalisationHelper.getString("data_panel_count_full_gc_pauses"));
			addEntry(LocalisationHelper.getString("data_panel_performance_fullgc"));
			addEntry("Percentage of time in full GC");//TODO localize
			addEntry("Full GC to GC");//TODO localize
		}

		public void setModel(GCModel model) {
			//LMA : add percentage of GC + Young to Full GC percentage
			double youngPausePercent = model.getGCPause().getSum() / model.getRunningTime() * 100;
			double fullGcPausePercent = model.getFullGCPause().getSum() / model.getRunningTime() * 100;
			double fullGcToGcPausePercent = fullGcPausePercent / (youngPausePercent + fullGcPausePercent) * 100;
			...
			updateValue(LocalisationHelper.getString("data_panel_performance_gc"),
					...
					? footprintFormatter.format(model.getFreedMemoryByGC().getSum()/model.getGCPause().getSum()) + "/s"
							: "n/a",
							model.getGCPause().getN() > 0);
			updateValue("Percentage of time in GC", throughputFormatter.format(youngPausePercent + fullGcPausePercent) + "%", true);//TODO localize
			...
			updateValue(LocalisationHelper.getString("data_panel_performance_fullgc"),
					...
					? footprintFormatter.format(model.getFreedMemoryByFullGC().getSum()/model.getFullGCPause().getSum()) + "/s"
							: "n/a",
							model.getFullGCPause().getN() > 0);
			updateValue("Percentage of time in full GC", throughputFormatter.format(fullGcPausePercent) + "%", true);//TODO localize
			updateValue("Full GC to GC", throughputFormatter.format(fullGcToGcPausePercent) + "%", true);//TODO localize
